...
class GamesController extends BaseController
{
    ...
    private function handleImageUpload()
    {
        ...
        if (move_uploaded_file($file['tmp_name'], $destination)) {
            ...
            return URLROOT . '/public/uploads/' . $filename;
        }
    ...
    public function store(): void
    {
        ...
        if ($_SERVER['REQUEST_METHOD'] !== 'POST') {
            header('Location: ' . URLROOT . '/games/add');
            exit;
        }
        ...
        if ($success) {
            ...
            header('Location: ' . URLROOT . '/inventory?success=added');
            exit;
        } else {
            // Show error
            $data['categories'] = $this->gameModel->getCategories();
    ...
    {
        if ($_SERVER['REQUEST_METHOD'] !== 'POST') {
            header('Location: ' . URLROOT . '/games/edit/' . $game_id);
            exit;
        }
        ...
        if ($success) {
            header('Location: ' . URLROOT . '/inventory?success=updated');
            exit;
        } else {
    ...
    public function delete($game_id): void
    {
        ...
        if (!\App\Core\Security::validateCSRFToken($_POST['csrf_token'] ?? '')) {
            header('Location: ' . URLROOT . '/inventory?error=invalid_token');
            exit;
        }
        
        $success = $this->gameModel->delete($game_id);
        
        header('Location: ' . URLROOT . '/inventory?success=deleted');
        exit;
    }
